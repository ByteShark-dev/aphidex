...
void main() {
  ...
  test(
    'only real g1 bosses keep boss phases and they match curated counts',
    () {
  ...
  test(
    'every curated g1 boss phase has metadata and visible combat content',
    () {
      for (final id in expectedBosses) {
        final phases = (entry(id)['bossPhases'] as List)
            .cast<Map<String, dynamic>>();

        for (final phase in phases) {
          expect(
            (phase['id'] as String?)?.isNotEmpty,
            isTrue,
            reason: '$id phase is missing an id',
          );
          expect(
            phase['label'],
            isA<Map>(),
            reason: '$id ${phase['id']} is missing a label',
          );
          expect(
            phase['summary'],
            isA<Map>(),
            reason: '$id ${phase['id']} is missing a summary',
          );
    ...
    for (final enemy in data) {
      for (final loot in (enemy['loot'] as List?) ?? const []) {
        expect(
          ...
          isTrue,
          reason:
              '${enemy['id']} has unsupported loot section ${loot['section']}',
        );
      }
    }
  });
  ...
  test(
    'g1 umbrella entries for O.R.C. and infused creatures stay explanatory',
    () {
      ...
      expect(orc['description']['en'], contains('Project O.R.C.'));
  ...
  test(
    'g1 system entries cover raids, MIX.R defenses, JavaMatic, and Spicy Coaltana',
    () {
  ...
  test('g1 weak points stay aligned with the curated creature table', () {
    ...
    for (final entryData in expectedWeakPoints.entries) {
      final weakPoint =
          entry(entryData.key)['weakPoint'] as Map<String, dynamic>;
